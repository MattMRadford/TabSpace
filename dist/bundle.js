(() => {
 ...
 -> X  - '${str}'  : (${form})  `);
  ...
  function displayAnalytics() {
    ...
    const daysUsed = Math.max(1, Math.floor((Date.now() - (analytics.lastUsed || Date.now())) / (1e3 * 60 * 60 * 24)));
    content.innerHTML = `
    <div class="stat-grid">
      <div class="stat-card">
    ...
    <div class="insights">
      <p>\u{1F4A1} You've organized ${analytics.tabsOrganized} tabs into ${analytics.groupsCreated} groups!</p>
      ${analytics.focusSessions > 0 ? `<p>\u{1F3AF} Focus mode helped you concentrate ${analytics.focusSessions} times</p>` : ""}
    </div>
  `;
    panel.style.display = "block";
  }
  var pendingTabIds = /* @__PURE__ */ new Set();
  var unsortedTabs = [];
  var chromeColors = [
    "grey",
    "blue",
  ...
  var emojiMap = {
    "\u{1F310}": ["web", "browser", "online", "internet", "url", "surfing", "tab", "chrome"],
    "\u{1F4BB}": ["code", "dev", "frontend", "backend", "programming", "javascript", "html", "css", "software", "terminal", "github", "api", "node", "react", "vue", "docker", "script"],
    "\u{1F527}": ["tool", "utility", "extension", "plugin", "config", "addon", "setup", "framework", "feature"],
    "\u{1F4DA}": ["study", "learn", "tutorial", "course", "school", "education", "research", "assignment", "notes", "docs", "reference", "reading", "book"],
    "\u{1F9E0}": ["workspace", "dashboard", "planner", "kanban", "organize", "system", "routine", "focus", "notion", "workflow", "project"],
    "\u{1F465}": ["reddit", "forum", "thread", "community", "discussion", "social", "chat", "platform", "group"],
    "\u{1F3A5}": ["video", "youtube", "stream", "watch", "movie", "media", "clip", "trailer", "netflix", "film", "series", "entertainment"],
    "\u{1F3A7}": ["audio", "music", "playlist", "spotify", "sound", "radio", "tune", "mp3", "track", "listen"],
    "\u{1F4F0}": ["news", "update", "politics", "world", "headlines", "report", "press", "briefing", "cnn", "bbc"],
    "\u{1F4C4}": ["document", "pdf", "note", "writing", "form", "presentation", "slide", "spreadsheet", "sheet", "docs"],
    "\u{1F4E7}": ["email", "inbox", "gmail", "outlook", "message", "compose", "mail", "reply", "draft"],
    "\u{1F4C5}": ["calendar", "event", "meeting", "schedule", "appointment", "agenda", "invite", "zoom", "date"],
    "\u{1F6CD}\uFE0F": ["shopping", "buy", "cart", "store", "amazon", "checkout", "product", "ecommerce", "deal", "discount", "coupon"],
    "\u{1F4AC}": ["chat", "text", "message", "telegram", "whatsapp", "slack", "discord", "ping", "notification"],
    "\u{1F3E6}": ["finance", "bank", "money", "investment", "stocks", "crypto", "wallet", "payment", "budget", "paypal", "tax", "account"],
    "\u{1F30D}": ["travel", "trip", "hotel", "flight", "map", "gps", "guide", "location", "airbnb", "journey"],
    "\u{1F4F8}": ["design", "figma", "creative", "image", "photo", "art", "graphic", "illustration", "icon", "ui", "ux", "portfolio"],
    "\u{1F3AE}": ["game", "gaming", "steam", "xbox", "ps5", "nintendo", "play", "controller", "quest", "match", "level"],
    "\u{1F52C}": ["science", "technology", "experiment", "biology", "chemistry", "physics", "data", "space", "robot", "ai", "nasa", "lab"],
    "\u{1F9EA}": ["test", "demo", "sandbox", "try", "prototype", "preview", "experiment", "beta", "mock"],
    "\u{1F512}": ["security", "privacy", "password", "login", "auth", "vpn", "secure", "encryption", "2fa"],
    "\u{1F4BC}": ["work", "job", "career", "resume", "portfolio", "linkedin", "office", "tasks", "roles", "company", "hiring"],
    "\u{1F4A1}": ["general", "info", "reference", "concept", "overview", "thought", "misc", "insight"],
    "\u{1F5C2}\uFE0F": ["folder", "files", "group", "collection", "storage", "default", "archive"]
  };
  function getNextColor(index3) {
    return chromeColors[index3 % chromeColors.length];
  }
  ...
  chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    chrome.tabs.get(tabId, (updatedTab) => {
      ...
      if (updatedTab.url.startsWith("chrome://") || updatedTab.url.includes("tabspace.html")) return;
  ...
  function triggerAIHighlight(state) {
    const aiBtn = document.getElementById("groupBtn");
    if (!aiBtn) return;
    if (state) {
    ...
    if (data.tabspaceGroups) {
      ...
      chrome.tabs.query({ currentWindow: true }, (tabs) => {
        ...
        for (const [groupName, titles] of Object.entries(lastGroupedTitles)) {
          updatedGroups[groupName] = titles.map(
            (title) => tabs.find((tab) => tab.title === title)
          ).filter(Boolean);
        }
        const nonEmptyGroups = Object.fromEntries(
          Object.entries(updatedGroups).filter(([, tabs2]) => tabs2.length > 0)
        );
  ...
  document.getElementById("refreshBtn").onclick = () => {
    ...
    chrome.tabs.query({ currentWindow: true }, (tabs) => {
      ...
      for (const groupName in lastGroupedTitles) {
        const idList = lastGroupedTitles[groupName].map((id) => parseInt(id, 10)).filter(Number.isInteger);
        updatedGroups[groupName] = tabs.filter((tab) => idList.includes(tab.id));
        console.log(`\u{1F50D} Refreshing group "${groupName}" with IDs:`, idList);
        console.log(`\u{1F3AF} Matched tabs:`, updatedGroups[groupName].map((t3) => t3.title));
      }
      const nonEmptyGroups = Object.fromEntries(
        Object.entries(updatedGroups).filter(([, tabs2]) => tabs2.length > 0)
      );
    ...
    tabs.forEach((tab) => {
      ...
      openBtn.onclick = () => {
        chrome.tabs.query({}, (allTabs) => {
          const match2 = allTabs.find((t3) => t3.url === tab.url);
          if (match2) {
            chrome.tabs.update(match2.id, { active: true });
            chrome.windows.update(match2.windowId, { focused: true });
          } else {
            chrome.tabs.create({ url: tab.url });
          }
        });
    ...
    chrome.tabs.query({ currentWindow: true }, async (tabs) => {
      const enrichedTabs = tabs.map((tab) => {
        ...
        return {
          id: tab.id,
          title: tab.title,
          // raw title for matching
          domain,
          // helpful for grouping
          enriched: `${tab.title} [${domain}]`
...
${JSON.stringify(enrichedTabs, null, 2)}
      ...
      const nonEmptyGroups = Object.fromEntries(
        Object.entries(groupedTabs).filter(([, tabs2]) => tabs2.length > 0)
      );
      renderGroupedTabs(nonEmptyGroups);
      document.getElementById("loadingSpinner").style.display = "none";
      unsortedTabs = [];
      ...
      for (const [groupName, tabs2] of Object.entries(groupedTabs)) {
        const tabIds = tabs2.filter((tab) => tab.id).map((tab) => tab.id);
        if (tabIds.length === 0) continue;
        const colorName = getNextColor(groupIndex);
        groupIndex++;
  ...
  function renderGroupedTabs(groups, groupColors = {}) {
    const sortedGroups = Object.entries(groups).sort(([, tabsA], [, tabsB]) => tabsB.length - tabsA.length);
    const tabList = document.getElementById("tabList");
    tabList.innerHTML = "";
  ...
  function getEmojiForGroup(name) {
    const terms = three_default(name).nouns().toSingular().out("array").map((t3) => t3.toLowerCase());
    let bestEmoji = "\u{1F4C1}";
    let bestScore = 0;
    for (const [emoji, keywords] of Object.entries(emojiMap)) {
      let score = 0;
      for (const term of terms) {
        for (const keyword of keywords) {
          if (term === keyword) score += 3;
          else if (term.includes(keyword) || keyword.includes(term)) score += 2;
          else if (levenshtein(term, keyword) <= 2) score += 1;
        }
      }
      if (score > bestScore) {
        bestScore = score;
  ...
  function levenshtein(a2, b) {
    const matrix = Array.from({ length: b.length + 1 }, (_2, i3) => [i3]);
    for (let i3 = 0; i3 <= a2.length; i3++) matrix[0][i3] = i3;
    for (let i3 = 1; i3 <= b.length; i3++) {
      for (let j2 = 1; j2 <= a2.length; j2++) {
        matrix[i3][j2] = b[i3 - 1] === a2[j2 - 1] ? matrix[i3 - 1][j2 - 1] : Math.min(matrix[i3 - 1][j2 - 1] + 1, matrix[i3][j2 - 1] + 1, matrix[i3 - 1][j2] + 1);
      }
    }
  ...
  document.getElementById("cleanupBtn").addEventListener("click", () => {
    chrome.tabs.query({ currentWindow: true }, (tabs) => {
      ...
      if (oldTabs.length === 0) {
        alert("No old tabs found to clean up! \u{1F389}");
        return;
      }
      const oldestTab = Math.min(...oldTabs.map((tab) => tab.lastAccessed || 0));
      const daysOld = Math.floor((now - oldestTab) / (24 * 60 * 60 * 1e3));
...
Close them?`
      ...
      if (confirmed) {
        ...
        chrome.tabs.remove(tabIds, () => {
          alert(`Cleaned up ${oldTabs.length} old tabs! \u{1F9F9}`);
          document.getElementById("refreshBtn")?.click();
        });
      }
    });
